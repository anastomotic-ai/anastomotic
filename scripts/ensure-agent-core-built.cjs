...
if (hostNodeTarget) {
  if (!isHostNodeRuntimeAvailable(hostNodeTarget)) {
    ...
    runPnpm(
      ['-F', '@anastomotic/desktop', 'download:nodejs', `--platform=${hostNodeTarget}`],
...
if (needsAgentCoreBuild) {
  console.log('Missing @anastomotic_ai/agent-core build outputs:');
  ...
  runPnpm(['-F', '@anastomotic_ai/agent-core', 'build'], 'Building @anastomotic_ai/agent-core...');

  missingOutputs = [...outputPaths].filter(
    (relativeOutputPath) => !fs.existsSync(path.join(agentCoreDir, relativeOutputPath)),
  );
  if (missingOutputs.length > 0) {
    console.error('Failed to produce required @anastomotic_ai/agent-core outputs:');
    for (const missingOutput of missingOutputs) {
...
if (missingMcpDistOutputs.length > 0) {
  ...
  runPnpm(
    ['-F', '@anastomotic/desktop', 'build:mcp-tools:dev'],
...
console.log('✓ @anastomotic_ai/agent-core and MCP dist outputs found');
process.exit(0);
