...
if (!url) {
  ...
  console.error(
    'Example: pnpm dev:remote https://anastomotic-app-preview-42.anastomotic.workers.dev',
  );
  process.exit(1);
}
...
const env = {
  ...process.env,
  ANASTOMOTIC_ROUTER_URL: url,
};

console.log('[dev:remote] Launching Electron → ' + url);

const electronArgs = ['-F', '@anastomotic/desktop', 'dev:remote'];
if (isCheck) {
