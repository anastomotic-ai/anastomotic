...
import type { FileAttachmentInfo } from '@anastomotic_ai/agent-core/common';
...
export function ExecutionPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeDebugMode = api.onDebugModeChange?.(({ enabled }) => {
  ...
  useEffect(() => {
    if (id) {
      ...
      api.getTodosForTask(id).then((todos) => {
    ...
    const unsubscribeTaskBatch = api.onTaskUpdateBatch?.((event) => {
    ...
    const unsubscribeDebugLog = api.onDebugLog((log) => {
  ...
  }, [followUp, attachments, api, sendFollowUp]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      void api.getSlackMcpOauthStatus().then((status) => {
        if (status.pendingAuthorization) {
          void api.logoutSlackMcp();
        }
      });
    }
  }, [
  ...
  const handlePauseAction = useCallback(async () => {
    ...
    try {
      ...
      const refreshedStatus = await api.getSlackMcpOauthStatus();
  ...
  }, [api, pauseAction, resumePausedTask, t]);
  ...
  const handlePickFiles = async () => {
    ...
    try {
      const newFiles = await api.pickFiles();
  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    try {
      const newAttachments = await api.processDroppedFiles(filePaths);
  ...
  const handleBugReport = useCallback(async () => {
    ...
    try {
      ...
      const result = await api.generateBugReport({
  ...
  const handleRepeatTask = useCallback(async () => {
    ...
    try {
      const newTask = await api.startTask({ prompt: currentTask.prompt });
