...
interface TaskState {
  ...
  sendFollowUp: (
    message: string,
    attachments?: import('@anastomotic_ai/agent-core/common').FileAttachmentInfo[],
  ) => Promise<boolean>;
...
export const useTaskStore = create<TaskState>((set, get) => ({
  ...
  sendFollowUp: async (
    ...
    try {
      ...
      const task = await api.resumeSession(sessionId, message, currentTask.id, attachments);
  ...
  removeFavorite: async (taskId: string) => {
    ...
    try {
      const api = getAnastomotic();
      await api.removeFavorite(taskId);
    } catch {
      if (removed) {
        set((state) => ({
...
if (typeof window !== 'undefined' && window.anastomotic) {
  window.anastomotic.onTaskProgress((progress: unknown) => {
  ...
  window.anastomotic.onTaskUpdate((event: unknown) => {
  ...
  window.anastomotic.onTaskSummary?.((data: { taskId: string; summary: string }) => {
    useTaskStore.getState().setTaskSummary(data.taskId, data.summary);
  });
  window.anastomotic.onTodoUpdate?.((data: { taskId: string; todos: TodoItem[] }) => {
    const state = useTaskStore.getState();
