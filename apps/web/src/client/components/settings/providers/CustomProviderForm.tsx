...
import type { ConnectedProvider, CustomCredentials } from '@anastomotic_ai/agent-core';
...
export function CustomProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const result = await api.testCustomConnection(baseUrl.trim(), trimmedKey);
      if (!result.success) {
      ...
      if (trimmedKey) {
        await api.addApiKey('custom', trimmedKey);
      } else {
        // Remove any previously stored key when connecting without one
        await api.removeApiKey('custom');
      }
