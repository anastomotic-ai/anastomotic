...
import { PROVIDER_META, DEFAULT_PROVIDERS } from '@anastomotic_ai/agent-core/common';
import {
  ModelSelector,
  ConnectButton,
  ConnectedControls,
...
export function ClassicProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (isOpenAI) {
        await api.setOpenAiBaseUrl(openAiBaseUrl.trim());
      }
      ...
      const validation = await api.validateApiKeyForProvider(providerId, apiKey.trim(), {
  ...
  const handleChatGptSignIn = async () => {
    ...
    try {
      const api = getAnastomotic();
      await api.loginOpenAiWithChatGpt();
