...
import type { HuggingFaceLocalCredentials } from '@anastomotic_ai/agent-core/common';
import {
  ConnectButton,
  ConnectedControls,
  ProviderFormHeader,
...
export function HuggingFaceProviderForm({
  ...
  useEffect(() => {
    const api = getAnastomotic();
    const unsub = api.onHuggingFaceDownloadProgress((progress) => {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const downloadResult = await api.downloadHuggingFaceModel(selectedModelId);
      ...
      const serverResult = await api.startHuggingFaceServer(selectedModelId);
  ...
  const handleDisconnect = async () => {
    try {
      const api = getAnastomotic();
      await api.stopHuggingFaceServer();
    } catch {
