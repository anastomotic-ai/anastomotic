...
export function ZaiProviderForm({
  ...
  const handleConnect = async () => {
    ...
    try {
      const api = getAnastomotic();
      const validation = await api.validateApiKeyForProvider('zai', apiKey.trim(), {
        region,
      });
      ...
      await api.addApiKey('zai', apiKey.trim());

      // Fetch models dynamically
      let fetchedModels: Array<{ id: string; name: string }> | undefined;
      if (providerConfig?.modelsEndpoint) {
        const fetchResult = await api.fetchProviderModels('zai', { zaiRegion: region });
