...
import { useAnastomotic } from '@/lib/anastomotic';
...
export function DaemonPanel() {
  ...
  const handleToggle = async () => {
    ...
    setSaving(true);
    try {
      await api.setRunInBackground(next);
  ...
  return (
    <div className="space-y-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
              Keep Anastomotic running in the system tray when the window is closed. Tasks continue
        ...
        {runInBackground && (
          <div className="mt-3 rounded-lg bg-primary/5 p-3">
            <p className="text-sm text-muted-foreground">
              Anastomotic will stay active in the system tray when the window is closed. Use the
              tray icon to show the window or quit the app.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="rounded-lg border border-border bg-card p-5">
        ...
        <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
          External clients (CLI tools, integrations, scheduled jobs) can send tasks to Anastomotic
        ...
        <div className="mt-4 space-y-2">
          ...
          <pre className="overflow-x-auto max-w-full rounded-md bg-muted px-3 py-2 text-xs font-mono text-foreground whitespace-pre-wrap break-all">
            {navigator.platform?.startsWith('Win')
              ? `echo {"jsonrpc":"2.0","id":1,"method":"task.start","params":{"prompt":"List files"}} | npx json-rpc-pipe "\\\\.\\pipe\\anastomotic-daemon"`
        ...
        <div className="mt-4 space-y-2">
          ...
          <pre className="overflow-x-auto max-w-full rounded-md bg-muted px-3 py-2 text-xs font-mono text-foreground whitespace-pre-wrap break-all">
            {navigator.platform?.startsWith('Win')
              ? `echo {"jsonrpc":"2.0","id":1,"method":"task.schedule","params":{"cron":"0 9 * * 1-5","prompt":"Check email and summarize"}} | npx json-rpc-pipe "\\\\.\\pipe\\anastomotic-daemon"`
