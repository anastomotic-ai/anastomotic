...
import type { KnowledgeNote, KnowledgeNoteType } from '@anastomotic_ai/agent-core';

type AnastomoticInstance = ReturnType<typeof getAnastomotic>;
...
export function useKnowledgeNotes(
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
      const loaded = await api.listKnowledgeNotes(workspaceId);
  ...
  }, [api, workspaceId, newType, newContent, loadNotes]);
  ...
  const handleEdit = useCallback(
    async (id: string) => {
      ...
      try {
        const updated = await api.updateKnowledgeNote(id, workspaceId, {
          type: editType,
    ...
    [api, workspaceId, editType, editContent, loadNotes, t],
  );

  const handleDelete = useCallback(
    async (id: string) => {
      try {
        const deleted = await api.deleteKnowledgeNote(id, workspaceId);
        if (workspaceIdRef.current !== workspaceId) {
          return;
        }
        if (!deleted) {
