...
import type { Skill } from '@anastomotic_ai/agent-core/common';
...
export function SkillsPanel({ refreshTrigger }: SkillsPanelProps) {
  ...
  useEffect(() => {
    if (!window.anastomotic) {
      logger.error('Anastomotic API not available');
      setLoading(false);
      return;
    }
    window.anastomotic
  ...
  const handleToggle = useCallback(
    async (id: string) => {
      ...
      if (!skill || !window.anastomotic) return;

      try {
        await window.anastomotic.setSkillEnabled(id, !skill.isEnabled);
  ...
  const handleDelete = useCallback(
    async (id: string) => {
      ...
      try {
        await window.anastomotic.deleteSkill(id);
  ...
  const handleEdit = useCallback(async (filePath: string) => {
    ...
    try {
      await window.anastomotic.openSkillInEditor(filePath);
    } catch (err) {
  ...
  const handleShowInFolder = useCallback(async (filePath: string) => {
    if (!window.anastomotic) return;
    try {
      await window.anastomotic.showSkillInFolder(filePath);
    } catch (err) {
  ...
  const handleResync = useCallback(async () => {
    if (!window.anastomotic || isResyncing) return;
    setIsResyncing(true);
    try {
      // Run resync and minimum delay in parallel so animation is visible
      const [updatedSkills] = await Promise.all([
        window.anastomotic.resyncSkills(),
