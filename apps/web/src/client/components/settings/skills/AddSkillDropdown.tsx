...
export function AddSkillDropdown({ onSkillAdded, onClose }: AddSkillDropdownProps) {
  ...
  const handleUploadSkill = async () => {
    ...
    try {
      setIsLoading(true);
      setUploadError(null);
      const folderPath = await window.anastomotic.pickSkillFolder();
      if (!folderPath) {
        setIsLoading(false);
        return; // User cancelled
      }
      await window.anastomotic.addSkillFromFolder(folderPath);
      onSkillAdded?.();
    } catch (err) {
  ...
  const handleImportFromGitHub = async () => {
    if (!gitHubUrl.trim() || !window.anastomotic) return;

    try {
      setIsLoading(true);
      setError(null);
      await window.anastomotic.addSkillFromGitHub(gitHubUrl);
      setGitHubUrl('');
        ...
        <DropdownMenuContent align="end" className="w-60">
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{t('skills.buildWithAnastomotic')}</span>
            </div>
