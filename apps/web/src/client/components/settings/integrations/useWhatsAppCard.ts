...
export function useWhatsAppCard(): WhatsAppCardState & WhatsAppCardActions {
  ...
  useEffect(() => {
    const unsubQR = api.onWhatsAppQR((data: { qr: string; expiresAt: number }) => {
      setQrCode(data.qr);
    ...
    const unsubStatus = api.onWhatsAppStatus((data: { status: string; phone?: string }) => {
