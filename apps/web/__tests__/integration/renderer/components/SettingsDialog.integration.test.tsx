...
describe('SettingsDialog Integration', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    mockAnastomotic.getConnectors.mockResolvedValue([]);
    mockAnastomotic.getSlackMcpOauthStatus.mockResolvedValue({
      connected: false,
      pendingAuthorization: false,
    });
    mockAnastomotic.loginSlackMcp.mockResolvedValue({ ok: true });
    mockAnastomotic.logoutSlackMcp.mockResolvedValue(undefined);
  });
  ...
  describe('dialog rendering', () => {
    ...
    it('should fetch initial data on open', async () => {
      ...
      await waitFor(() => {
        expect(mockAnastomotic.getProviderSettings).toHaveBeenCalled();
      });
    });
  ...
  describe('provider active state', () => {
    ...
    it('should call setActiveProvider when a ready provider connects (regression test)', async () => {
      ...
      mockAnastomotic.getProviderSettings = vi.fn().mockResolvedValue({
        activeProviderId: 'anthropic',
        connectedProviders: {
          anthropic: {
            providerId: 'anthropic',
  ...
  describe('connectors tab', () => {
    it('should render the Slack authentication card', async () => {
      ...
      expect(mockAnastomotic.getSlackMcpOauthStatus).toHaveBeenCalled();
    });

    it('should authenticate Slack from the connectors tab', async () => {
      mockAnastomotic.getSlackMcpOauthStatus
      ...
      await waitFor(() => {
        expect(mockAnastomotic.loginSlackMcp).toHaveBeenCalled();
      });
