...
import type { Task, TaskStatus } from '@anastomotic_ai/agent-core';
...
describe('Home Page Integration', () => {
  beforeEach(() => {
    ...
    mockAnastomotic.getProviderSettings.mockResolvedValue({
      activeProviderId: 'anthropic',
      connectedProviders: {
        anthropic: {
          providerId: 'anthropic',
  ...
  describe('initial render', () => {
    it('should render the main heading', () => {
      ...
      // Assert
      expect(
        screen.getByRole('heading', { name: /what would you like to do today/i }),
      ).toBeInTheDocument();
    });
  ...
  describe('task input integration', () => {
    ...
    it('should check for provider settings before submitting task', async () => {
      ...
      await waitFor(() => {
        expect(mockAnastomotic.isE2EMode).toHaveBeenCalled();
      });
    });
    ...
    it('should not submit empty task', async () => {
      ...
      await waitFor(() => {
        expect(mockAnastomotic.isE2EMode).not.toHaveBeenCalled();
