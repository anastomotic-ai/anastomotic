 ...
 * Tests store actions with mocked window.anastomotic API
...
// Mock anastomotic API
const mockAnastomotic = {
  startTask: vi.fn(),
  cancelTask: vi.fn(),
  interruptTask: vi.fn(),
  resumeSession: vi.fn(),
...
// Mock the anastomotic module
vi.mock('@/lib/anastomotic', () => ({
  getAnastomotic: () => mockAnastomotic,
}));

// Mock window.anastomotic for global subscriptions
...
describe('taskStore Integration', () => {
  ...
  describe('startTask', () => {
    it('should call startTask API and update state on success', async () => {
      ...
      mockAnastomotic.startTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockAnastomotic.startTask).toHaveBeenCalledWith(config);
    ...
    it('should set error state on failure', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockAnastomotic.startTask.mockRejectedValueOnce(new Error('API Error'));
    ...
    it('should handle non-Error exceptions gracefully', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockAnastomotic.startTask.mockRejectedValueOnce('String error');
    ...
    it('should update existing task if same ID', async () => {
      ...
      mockAnastomotic.startTask.mockResolvedValueOnce(updatedTask);
  ...
  describe('sendFollowUp', () => {
    it('should show error if no active task', async () => {
      ...
      expect(mockAnastomotic.logEvent).toHaveBeenCalledWith(
    ...
    it('should start fresh task for interrupted task without session', async () => {
      ...
      mockAnastomotic.startTask.mockResolvedValueOnce(newTask);
      ...
      // Assert
      expect(mockAnastomotic.startTask).toHaveBeenCalled();
    });
    ...
    it('should resume session when task has sessionId', async () => {
      ...
      mockAnastomotic.resumeSession.mockResolvedValueOnce(resumedTask);
      ...
      // Assert
      expect(mockAnastomotic.resumeSession).toHaveBeenCalledWith(
        'session-abc',
        'Continue please',
        'task-123',
      );
    ...
    it('should add user message optimistically', async () => {
      ...
      mockAnastomotic.resumeSession.mockResolvedValueOnce(
    ...
    it('should handle resumeSession failure', async () => {
      ...
      mockAnastomotic.resumeSession.mockRejectedValueOnce(new Error('Resume failed'));
  ...
  describe('cancelTask', () => {
    it('should call cancelTask API and update status', async () => {
      ...
      mockAnastomotic.cancelTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockAnastomotic.cancelTask).toHaveBeenCalledWith('task-123');
    ...
    it('should do nothing when no current task', async () => {
      ...
      // Assert
      expect(mockAnastomotic.cancelTask).not.toHaveBeenCalled();
    });
  });
  ...
  describe('interruptTask', () => {
    it('should call interruptTask API for running task', async () => {
      ...
      mockAnastomotic.interruptTask.mockResolvedValueOnce(undefined);

      // Act
      await useTaskStore.getState().interruptTask();

      // Assert
      expect(mockAnastomotic.interruptTask).toHaveBeenCalledWith('task-123');
    });
    ...
    it('should not call API for non-running task', async () => {
      ...
      // Assert
      expect(mockAnastomotic.interruptTask).not.toHaveBeenCalled();
    });
  ...
  describe('error state management', () => {
    it('should clear error on successful task start', async () => {
      ...
      mockAnastomotic.startTask.mockResolvedValueOnce(createMockTask('task-123'));
  ...
  describe('loadTasks', () => {
    it('should load tasks from API', async () => {
      ...
      mockAnastomotic.listTasks.mockResolvedValueOnce(mockTasks);
      ...
      // Assert
      expect(mockAnastomotic.listTasks).toHaveBeenCalled();
  ...
  describe('loadTaskById', () => {
    it('should load specific task and set as current', async () => {
      ...
      mockAnastomotic.getTask.mockResolvedValueOnce(mockTask);
      ...
      // Assert
      expect(mockAnastomotic.getTask).toHaveBeenCalledWith('task-123');
      expect(state.currentTask).toEqual(mockTask);
      expect(state.error).toBeNull();
    });
    ...
    it('should set error when task not found', async () => {
      // Arrange
      const { useTaskStore } = await import('@/stores/taskStore');
      mockAnastomotic.getTask.mockResolvedValueOnce(null);
  ...
  describe('deleteTask', () => {
    it('should delete task and remove from list', async () => {
      ...
      mockAnastomotic.deleteTask.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockAnastomotic.deleteTask).toHaveBeenCalledWith('task-2');
      expect(state.tasks).toHaveLength(2);
  ...
  describe('clearHistory', () => {
    it('should clear all tasks', async () => {
      ...
      mockAnastomotic.clearTaskHistory.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockAnastomotic.clearTaskHistory).toHaveBeenCalled();
      expect(state.tasks).toEqual([]);
    });
  });
      ...
      useTaskStore.setState({
        ...
        permissionRequests: {
          'task-1': {
            ...
            taskId: 'task-1',
            type: 'file',
          } as import('@anastomotic_ai/agent-core/common').PermissionRequest,
        },
  ...
  describe('respondToPermission', () => {
    it('should call API and clear permission request', async () => {
      ...
      mockAnastomotic.respondToPermission.mockResolvedValueOnce(undefined);
      ...
      // Assert
      expect(mockAnastomotic.respondToPermission).toHaveBeenCalledWith(response);
