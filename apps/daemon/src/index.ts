...
async function main(): Promise<void> {
  ...
  // 5. Create services
  const userDataPath = args.dataDir || path.join(homedir(), '.anastomotic');
  const mcpToolsPath =
    process.env.MCP_TOOLS_PATH ||
    path.resolve(__dirname, '..', '..', '..', 'packages', 'agent-core', 'mcp-tools');
  const isPackaged = process.env.ANASTOMOTIC_IS_PACKAGED === '1';
  const taskService = new TaskService(storage, {
    userDataPath,
    mcpToolsPath,
    isPackaged,
    resourcesPath: process.env.ANASTOMOTIC_RESOURCES_PATH,
    appPath: process.env.ANASTOMOTIC_APP_PATH,
  });
  ...
  process.env.ANASTOMOTIC_DAEMON_AUTH_TOKEN = authToken;
  if (permPorts.permissionPort) {
    process.env.ANASTOMOTIC_PERMISSION_API_PORT = String(permPorts.permissionPort);
  }
  if (permPorts.questionPort) {
    process.env.ANASTOMOTIC_QUESTION_API_PORT = String(permPorts.questionPort);
  }
  if (thoughtPort) {
    process.env.ANASTOMOTIC_THOUGHT_STREAM_PORT = String(thoughtPort);
  }
