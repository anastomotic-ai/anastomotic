...
export class TaskService extends EventEmitter {
  ...
  private async onBeforeStart(): Promise<void> {
    ...
    const permissionApiPort = process.env.ANASTOMOTIC_PERMISSION_API_PORT
      ? parseInt(process.env.ANASTOMOTIC_PERMISSION_API_PORT, 10)
      : undefined;
    const questionApiPort = process.env.ANASTOMOTIC_QUESTION_API_PORT
      ? parseInt(process.env.ANASTOMOTIC_QUESTION_API_PORT, 10)
      : undefined;
    ...
    const result = generateConfig({
      ...
      permissionApiPort,
      questionApiPort,
      authToken: process.env.ANASTOMOTIC_DAEMON_AUTH_TOKEN,
