...
if (!isRemote && !env.ANASTOMOTIC_ROUTER_URL) {
  env.ANASTOMOTIC_ROUTER_URL = 'http://localhost:5173';
}
if (isClean) {
  env.CLEAN_START = '1';
}
...
function ensureNativeModules(commandEnv) {
  const forceRebuild = process.env.ANASTOMOTIC_FORCE_ELECTRON_REBUILD === '1';
