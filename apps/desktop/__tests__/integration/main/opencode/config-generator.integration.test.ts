...
// Note: PERMISSION_API_PORT and QUESTION_API_PORT are now imported from @anastomotic/shared
// by config-generator.ts, so no mock needed here

// Mock @anastomotic_ai/agent-core (uses SQLite which requires native module)
// Note: generateConfig mock creates real files in temp directory for integration testing
vi.mock('@anastomotic_ai/agent-core', async () => {
...
Use AskUserQuestion tool for user interaction.`,
        ...
        return {
          configPath,
          systemPrompt: config.agent.anastomotic.prompt,
    ...
    ANASTOMOTIC_AGENT_NAME: 'anastomotic',
...
describe('OpenCode Config Generator Integration', () => {
  ...
  describe('generateOpenCodeConfig()', () => {
    ...
    it('should write config file with correct structure', async () => {
      ...
      expect(config.default_agent).toBe('anastomotic');
    ...
    it('should include anastomotic agent configuration', async () => {
      // Act
      const { generateOpenCodeConfig } = await import('@main/opencode/config-generator');
      const configPath = await generateOpenCodeConfig();
      ...
      const agent = config.agent['anastomotic'];
    ...
    it('should include platform-specific environment instructions', async () => {
      ...
      // Assert
      const config = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      const prompt = config.agent['anastomotic'].prompt;
    ...
    it('should include enabled connected connectors with access tokens', async () => {
      ...
      const { generateConfig } = await import('@anastomotic_ai/agent-core');
  ...
  describe('ANASTOMOTIC_AGENT_NAME Export', () => {
    it('should export the agent name constant', async () => {
      // Act
      const { ANASTOMOTIC_AGENT_NAME } = await import('@main/opencode/config-generator');

      // Assert
      expect(ANASTOMOTIC_AGENT_NAME).toBe('anastomotic');
    });
  });
