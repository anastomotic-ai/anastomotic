    ...
    void (async () => {
      try {
        ...
        try {
          ...
          const dataHome =
            process.env.XDG_DATA_HOME || pathModule.join(osModule.homedir(), '.local', 'share');
          const authPath = pathModule.join(dataHome, 'opencode', 'auth.json');
          ...
          fs.mkdirSync(pathModule.dirname(authPath), { recursive: true });
