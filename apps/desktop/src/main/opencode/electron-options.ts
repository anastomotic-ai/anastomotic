...
import type { TaskManagerOptions, TaskCallbacks } from '@anastomotic_ai/agent-core';
import type { TaskConfig } from '@anastomotic_ai/agent-core';
import { DEV_BROWSER_PORT } from '@anastomotic_ai/agent-core';
import {
  getAzureEntraToken,
...
import { getModelDisplayName } from '@anastomotic_ai/agent-core';
import type {
...
export function getBundledOpenCodeVersion(): string | null {
  ...
  try {
    ...
    // See: https://github.com/RajdeepKushwaha5/accomplish/issues/596
  ...
  if (!bundledNode) {
    throw new Error(
      '[OpenCode CLI] Bundled Node.js path is missing. ' +
        'Run "pnpm -F @anastomotic/desktop download:nodejs" and rebuild before launching.',
    );
  }
