...
const APP_DATA_NAME = 'Anastomotic';
app.setPath('userData', path.join(app.getPath('appData'), APP_DATA_NAME));

if (process.platform === 'win32') {
  app.setAppUserModelId('ai.anastomotic.desktop');
}

import { registerIPCHandlers } from './ipc/handlers';
import { FutureSchemaError } from '@anastomotic_ai/agent-core';
...
const ROUTER_URL = process.env.ANASTOMOTIC_ROUTER_URL;
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, commandLine) => {
    if (mainWindow) {
      ...
      if (process.platform === 'win32') {
        const protocolUrl = commandLine.find((arg) => arg.startsWith('anastomotic://'));
        if (protocolUrl) {
          logMain('INFO', `[Main] Received protocol URL from second-instance: ${protocolUrl}`);
          if (protocolUrl.startsWith('anastomotic://callback/mcp')) {
            mainWindow.webContents.send('auth:mcp-callback', protocolUrl);
          } else if (protocolUrl.startsWith('anastomotic://callback')) {
    ...
    try {
      ...
      if (err instanceof FutureSchemaError) {
        await dialog.showMessageBox({
          ...
          message: `This data was created by a newer version of Anastomotic (schema v${err.storedVersion}).`,
          detail: `Your app supports up to schema v${err.appVersion}. Please update Anastomotic to continue.`,
          buttons: ['Quit'],
        });
        app.quit();
        return;
...
if (process.platform === 'win32' && !app.isPackaged) {
  app.setAsDefaultProtocolClient('Anastomotic', process.execPath, [path.resolve(process.argv[1])]);
} else {
  app.setAsDefaultProtocolClient('Anastomotic');
}

function handleProtocolUrlFromArgs(): void {
  if (process.platform === 'win32') {
    const protocolUrl = process.argv.find((arg) => arg.startsWith('anastomotic://'));
    if (protocolUrl) {
      app.whenReady().then(() => {
        setTimeout(() => {
...
app.on('open-url', (event, url) => {
  event.preventDefault();
  if (url.startsWith('anastomotic://callback/mcp')) {
    mainWindow?.webContents?.send('auth:mcp-callback', url);
  } else if (url.startsWith('anastomotic://callback')) {
