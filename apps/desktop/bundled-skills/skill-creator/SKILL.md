...
## Saving Skills in Anastomotic

**IMPORTANT:** When creating skills in Anastomotic, you can ONLY create "custom" skills. You CANNOT create "official" skills - those are bundled with the app and managed by the Anastomotic team.
...
Skills must be saved to the Anastomotic user data directory under a `skills` folder:

**macOS:** `~/Library/Application Support/Anastomotic/skills/<skill-name>/SKILL.md`
**Windows:** `%APPDATA%\Anastomotic\skills\<skill-name>\SKILL.md`
**Linux:** `~/.config/Anastomotic/skills/<skill-name>/SKILL.md`
...
1. **Use the base path from the task prompt** when present.
   - If no path is provided, detect the OS and use:
     - macOS: `~/Library/Application Support/Anastomotic/skills/`
     - Windows: `%APPDATA%\Anastomotic\skills\`
     - Linux: `~/.config/Anastomotic/skills/`
...
5. **The skill is automatically detected** - Anastomotic scans this directory on startup and syncs new skills to its database. The skill will appear in Settings > Skills as a "Custom" skill.

### Skill Frontmatter Rules

For custom skills in Anastomotic:
