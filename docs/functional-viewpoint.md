# Functional Viewpoint — Anastomotic Architecture
...
graph TB
  ...
  subgraph ELECTRON["Anastomotic Desktop App"]
    direction TB
...
**Key takeaway:** Anastomotic never calls LLMs directly. It orchestrates OpenCode (a PTY subprocess) which does all AI interaction and file operations. Anastomotic's role is configuration, gating, completion enforcement, and UI.
...
graph TB
  ...
  subgraph BRIDGE["Preload Bridge"]
    CB["contextBridge<br/><i>window.anastomotic</i>"]
  end
...
graph TB
  subgraph SOURCES["Skill Sources"]
    ...
    USER_DIR["User Skills<br/><i>~/.../Anastomotic/skills/</i>"]
...
| **Preload Bridge**           | desktop    | `contextBridge.exposeInMainWorld('anastomotic', ...)`               | ~70 API methods exposed to renderer                                          |
