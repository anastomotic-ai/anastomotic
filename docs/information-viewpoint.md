...
This document captures all stateful data, information models, state machines, and data flows within the Anastomotic system. It follows the Rozanski & Woods Information Viewpoint to document data ownership, lifecycle, and structure.
...
- SQLite DB: `~/Library/Application Support/Anastomotic/anastomotic.db` (prod) / `anastomotic-dev.db` (dev)
- Secure Storage: `~/Library/Application Support/Anastomotic/secure-storage.json`
- Logs: `~/Library/Application Support/Anastomotic/logs/`
- Skills: `~/Library/Application Support/Anastomotic/skills/`
- OpenCode configs: `~/Library/Application Support/Anastomotic/opencode/`
...
flowchart LR
    subgraph Persistent["Persistent Storage"]
        direction TB
        DB[(SQLite DB<br/>anastomotic.db)]
...
Location: `~/Library/Application Support/Anastomotic/` (macOS)

All files and folders found in this directory at runtime:

### Anastomotic Application Data

| Path                     | Owner               | Description                                                                                                                                                                         |
| ------------------------ | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `anastomotic.db`         | agent-core (SQLite) | Main database — tasks, messages, todos, providers, settings, skills, connectors. All application state.                                                                             |
| `anastomotic.db-wal`     | SQLite engine       | Write-Ahead Log — buffered writes not yet checkpointed to main DB. Auto-managed.                                                                                                    |
| `anastomotic.db-shm`     | SQLite engine       | Shared memory index for WAL. Enables concurrent readers. Auto-managed.                                                                                                              |
| `anastomotic-dev.db`     | agent-core (SQLite) | Development-mode database (used when running `pnpm dev`). Same schema as prod.                                                                                                      |
| `anastomotic-dev.db-wal` | SQLite engine       | WAL for dev database.                                                                                                                                                               |
| `anastomotic-dev.db-shm` | SQLite engine       | Shared memory for dev database.                                                                                                                                                     |
...
These are standard Chromium storage directories created automatically by Electron. Not managed by Anastomotic code.

| Path                                                  | Owner           | Description                                                                                      |
| ----------------------------------------------------- | --------------- | ------------------------------------------------------------------------------------------------ |
| `Cache/`                                              | Chromium        | HTTP cache for the renderer process (the Anastomotic UI webview).                                |
| `Code Cache/`                                         | Chromium        | Compiled/cached JavaScript bytecode for faster UI startup.                                       |
| `Cookies`                                             | Chromium        | Cookie database for the Electron renderer (Anastomotic UI, not the automation browser).          |
...
| `IndexedDB/`                                          | Chromium        | Browser IndexedDB storage for the Anastomotic UI renderer.                                       |
| `Local Storage/`                                      | Chromium        | `localStorage` for the Anastomotic React app (theme preference synced here).                     |
| `Session Storage/`                                    | Chromium        | `sessionStorage` for the Anastomotic React app.                                                  |
...
| Scenario                           | Look at                                                                           |
| ---------------------------------- | --------------------------------------------------------------------------------- |
| Task not completing / wrong status | `anastomotic.db` → `tasks` table (status, session_id)                             |
| Messages missing or corrupt        | `anastomotic.db` → `task_messages` table (sort_order, content)                    |
| Provider connection issues         | `anastomotic.db` → `providers` table (connection_status, credentials_type)        |
