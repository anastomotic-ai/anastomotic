# Anastomotic — System Context Diagram
...
graph TB
    ...
    Anastomotic["<b>Anastomotic</b><br/><i>[Software System]</i><br/><br/>AI desktop agent that automates<br/>tasks via natural language"]:::system
    ...
    User -->|"natural language tasks<br/>+ approvals"| Anastomotic
    Anastomotic -->|"progress · results<br/>· permission requests"| User
    Anastomotic <-->|"LLM prompts / responses"| AI
    Anastomotic <-->|"read · write · organize"| FS
    Anastomotic -->|"browse · click · extract"| BROWSER
    Anastomotic <-->|"remote tool calls"| MCP
    Anastomotic -->|"voice input"| VOICE
    Anastomotic -->|"download skills"| SKILLS
```
...
graph TB
    ...
    Anastomotic["<b>Anastomotic</b><br/><i>[Software System]</i><br/><br/>AI desktop agent that automates<br/>file management, document creation,<br/>and browser tasks locally"]:::system
    ...
    User -- "Describes tasks,<br/>approves file operations,<br/>answers questions" --> Anastomotic
    Anastomotic -- "Shows progress,<br/>asks permissions,<br/>presents results" --> User

    Anastomotic -- "Sends prompts,<br/>receives AI responses<br/>(via OpenCode CLI)" --> CloudAI
    Anastomotic -- "Sends prompts,<br/>receives AI responses<br/>(via OpenCode CLI)" --> LocalAI

    Anastomotic -- "Reads, creates, moves,<br/>renames, deletes files<br/>(with user permission)" --> FileSystem

    Anastomotic -- "Launches, navigates,<br/>clicks, fills forms,<br/>extracts data" --> ChromeBrowser
    ChromeBrowser -- "Accesses" --> SaaS

    Anastomotic -- "Connects via OAuth 2.0,<br/>calls remote tools" --> MCPRemote

    Anastomotic -- "Transcribes<br/>voice input" --> ElevenLabs
    Anastomotic -- "Downloads<br/>skill definitions" --> GitHub
```
...
1. **Local-first**: Anastomotic runs entirely on the user's machine. No Anastomotic backend exists. Data stays local.

2. **Bring-your-own-AI**: Users provide their own API keys or run local models. Anastomotic is a tool, not a service.
