# Anastomotic Task Flow — Phase-by-Phase Sequence Diagrams
...
sequenceDiagram
    ...
    React->>Preload: anastomotic.startTask({ prompt })
...
sequenceDiagram
    ...
    OCA->>OCA: buildCliArgs(config)<br/>→ ["run","--format","json",<br/>"--model","anthropic/claude-sonnet",<br/>"--agent","anastomotic",<br/>"Please organize..."]
...
sequenceDiagram
    ...
    React->>Preload: anastomotic.respondToPermission(<br/>{ decision:'allow' })
...
sequenceDiagram
    ...
    React->>Preload: anastomotic.resumeSession(<br/>'sess_abc123',<br/>'Leave the pictures as is',<br/>'tsk_001')
    ...
    OCA->>PTY: pty.spawn('opencode run<br/>--format json --model claude-sonnet<br/>--session sess_abc123<br/>--agent anastomotic<br/>"Leave the pictures as is"')
