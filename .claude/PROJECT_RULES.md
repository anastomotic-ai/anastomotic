...
Single source of truth for all Anastomotic development rules.
...
When a PR adds a new `window.anastomotic.*` method, verify it is typed in `anastomotic.ts`
...
```bash
pnpm -F @anastomotic/web test:integration
```
