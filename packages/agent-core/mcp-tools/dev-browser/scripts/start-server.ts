...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'Anastomotic', 'dev-browser');
  } else if (process.platform === 'win32') {
    return join(process.env.APPDATA || homeDir, 'Anastomotic', 'dev-browser');
  } else {
    return join(homeDir, '.anastomotic', 'dev-browser');
  }
}
...
const ANASTOMOTIC_HTTP_PORT = parseInt(process.env.DEV_BROWSER_PORT || '9224', 10);
const ANASTOMOTIC_CDP_PORT = parseInt(process.env.DEV_BROWSER_CDP_PORT || '9225', 10);

if (
  !Number.isFinite(ANASTOMOTIC_HTTP_PORT) ||
  ANASTOMOTIC_HTTP_PORT < 1 ||
  ANASTOMOTIC_HTTP_PORT > 65535
) {
  throw new Error(
...
if (
  !Number.isFinite(ANASTOMOTIC_CDP_PORT) ||
  ANASTOMOTIC_CDP_PORT < 1 ||
  ANASTOMOTIC_CDP_PORT > 65535
) {
  throw new Error(
...
try {
  const res = await fetch(`http://localhost:${ANASTOMOTIC_HTTP_PORT}`, {
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          const output = execSync(`netstat -ano | findstr :${ANASTOMOTIC_HTTP_PORT}`, {
            encoding: 'utf-8',
          });
          ...
          const pid = execSync(`lsof -ti:${ANASTOMOTIC_HTTP_PORT}`, { encoding: 'utf-8' }).trim();
          if (pid) {
            execSync(`kill -9 ${pid}`);
          }
        }
      ...
      console.log(`Launch server already running on port ${ANASTOMOTIC_HTTP_PORT}`);
      process.exit(0);
    }
  }
} catch {
...
try {
  if (process.platform === 'win32') {
    const output = execSync(`netstat -ano | findstr :${ANASTOMOTIC_CDP_PORT}`, {
      encoding: 'utf-8',
    });
    const match = output.match(/LISTENING\s+(\d+)/);
    if (match) {
      const pid = match[1];
      console.log(
        `Cleaning up stale Chrome process on CDP port ${ANASTOMOTIC_CDP_PORT} (PID: ${pid})`,
      );
      execSync(`taskkill /F /PID ${pid}`, { stdio: 'ignore' });
    }
  } else {
    const pid = execSync(`lsof -ti:${ANASTOMOTIC_CDP_PORT}`, { encoding: 'utf-8' }).trim();
    if (pid) {
      console.log(
        `Cleaning up stale Chrome process on CDP port ${ANASTOMOTIC_CDP_PORT} (PID: ${pid})`,
      );
...
async function startServer(retry = false): Promise<void> {
  try {
    const server = await serve({
      port: ANASTOMOTIC_HTTP_PORT,
      cdpPort: ANASTOMOTIC_CDP_PORT,
      headless,
      profileDir,
