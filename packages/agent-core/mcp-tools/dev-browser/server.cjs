...
if (!process.env.NODE_BIN_PATH) {
  ...
  log('Run "pnpm -F @anastomotic/desktop download:nodejs" and rebuild before launching.');
  process.exit(1);
}
...
if (!fs.existsSync(bundledServer)) {
  ...
  log('Run "pnpm -F @anastomotic/desktop build:mcp-tools:dev" before starting the app.');
  process.exit(1);
}
