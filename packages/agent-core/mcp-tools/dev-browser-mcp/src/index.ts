...
console.error(
  '[dev-browser-mcp] ANASTOMOTIC_TASK_ID:',
  process.env.ANASTOMOTIC_TASK_ID || '(not set)',
);
...
async function loadToolDebug(): Promise<void> {
  const debugPath = process.env.ANASTOMOTIC_TOOL_DEBUG_PATH;
  if (debugPath) {
    ...
    console.error('[dev-browser-mcp] ANASTOMOTIC_TOOL_DEBUG_PATH not set, tool debug disabled');
  }
}
await loadToolDebug();
  ...
  try {
    ...
    const frameHandler = async (event: { data: string; sessionId: number }) => {
      try {
        ...
        const taskId = process.env.ANASTOMOTIC_TASK_ID || 'default';
        console.log(
          JSON.stringify({
            type: 'browser-frame',
            taskId,
