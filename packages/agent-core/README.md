# @anastomotic_ai/agent-core

Core library for the Anastomotic desktop automation assistant. Provides factory-based APIs for task management, persistent storage, permission handling, logging, and more.

## Usage

This package is internal to the Anastomotic monorepo and is consumed via workspace dependencies (for example, `"@anastomotic_ai/agent-core": "workspace:*"`).

## Quick Start

```typescript
import { createStorage, createTaskManager } from '@anastomotic_ai/agent-core';

// Initialize storage (SQLite + encrypted secure storage)
const storage = createStorage({
  databasePath: '/path/to/anastomotic.db',
...
```typescript
import { createTaskManager } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createStorage } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createPermissionHandler } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createThoughtStreamHandler } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createLogWriter } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createSkillsManager } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { createSpeechService } from '@anastomotic_ai/agent-core';
```
...
```typescript
import { generateConfig, type BrowserConfig } from '@anastomotic_ai/agent-core';
